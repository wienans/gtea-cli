...
function resolveIssueRepository(flags: ParsedIssueFlags, context: ResolvedCliExecutionContext): { repository?: RepositoryContext; error?: CliResult } {
  if (flags.repository !== undefined) {
    return parseRepositoryTarget(flags.repository, context);
  }

  return resolveRepositoryFromGit(context);
}

function resolveOptionalToken(hostname: string, context: ResolvedCliExecutionContext): string | undefined {
...
export async function executeIssueCommand(args: string[], context: ResolvedCliExecutionContext): Promise<CliResult | undefined> {
  ...
  const repositoryResult = resolveIssueRepository(parsedFlags.flags, context);
